...
class TodoControllerTest {
    ...
    @Test
    public void should_update_todo_done_when_perform_put() throws Exception {
        // given
        String updatedTodo = "{\n" +
                "    \"done\": true\n" +
                "}";
        // when & then
        client.perform(MockMvcRequestBuilders.put("/todos/{id}", savedTodoId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(updatedTodo))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.context").value("todo test"))
                .andExpect(MockMvcResultMatchers.jsonPath("$.done").value(true));
    }


    @Test
    public void should_update_todo_context_when_perform_put() throws Exception {
        // given
        String updatedTodo = "{\n" +
                "    \"context\": \"todo put context field test\"\n" +
                "}";
        // when & then
        client.perform(MockMvcRequestBuilders.put("/todos/{id}", savedTodoId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(updatedTodo))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.context").value("todo put context field test"))
                .andExpect(MockMvcResultMatchers.jsonPath("$.done").value(false));
    }
}
